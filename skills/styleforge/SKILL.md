...
可选第二参数指定要生成的资产（逗号分隔，默认 `logo,social_square,banner`，可选 `logo,banner,social_square,product_mockup,business_card`）：
...
- 每个**通过**的资产一行 `MEDIA:<absolute_path_to_png>`（Logo / Banner / Social Square …）
